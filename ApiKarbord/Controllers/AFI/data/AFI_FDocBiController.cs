...
namespace ApiKarbord.Controllers.AFI.data
{
    public class AFI_FDocBiController : ApiController
    {
        ...
        public async Task<IHttpActionResult> PutAFI_FDocBi(string ace, string sal, string group, long BandNo, AFI_FDocBi aFI_FDocBi)
        {
            ...
            string res = UnitDatabase.TestAcount(DBase, dataAccount[3], ace, group, UnitPublic.ModeCodeConnection(aFI_FDocBi.ModeCode));
            if (res == "")
            {
                try
                {
                    string sql = string.Format(CultureInfo.InvariantCulture,
                        ...
                        dBName,
                        aFI_FDocBi.LFctSerialNumber ?? 0,
                        aFI_FDocBi.InvSerialNumber ?? 0,
                        aFI_FDocBi.LinkNumber ?? 0,
                        aFI_FDocBi.LinkYear ?? 0,
                        aFI_FDocBi.LinkBandNo ?? 0,
                        aFI_FDocBi.LinkProg ?? ""
                        );
        ...
        public async Task<IHttpActionResult> DeleteAFI_FDocBi(string ace, string sal, string group, long SerialNumber, int BandNo, string ModeCode, string FlagLog)
        {
            ...
            var dataAccount = UnitDatabase.ReadUserPassHeader(this.Request.Headers);
            var DBase = UnitDatabase.dataDB.Where(p => p.UserName.ToUpper() == dataAccount[0].ToUpper() && p.Password == dataAccount[1]).Single();
            string res = UnitDatabase.TestAcount(DBase, dataAccount[3], ace, group, ModeCode);
            if (res == "")
            {
                try
                {
        ...
        public async Task<IHttpActionResult> PostAFI_SaveAllDocB(string ace, string sal, string group, long serialNumber, [FromBody]List<AFI_FDocBi> AFI_FDocBi)
        {
            ...
            var dataAccount = UnitDatabase.ReadUserPassHeader(this.Request.Headers);
            var DBase = UnitDatabase.dataDB.Where(p => p.UserName.ToUpper() == dataAccount[0].ToUpper() && p.Password == dataAccount[1]).Single();
            string res = UnitDatabase.TestAcount(DBase, dataAccount[3], ace, group, AFI_FDocBi[0].ModeCode);
            if (res == "")
            {
                int value;
                int i = 0;
        ...
        public async Task<IHttpActionResult> PostAFI_Convert(string ace, string sal, string group, ConvertObject ConvertObject)
        {
            ...
            var dataAccount = UnitDatabase.ReadUserPassHeader(this.Request.Headers);
            var DBase = UnitDatabase.dataDB.Where(p => p.UserName.ToUpper() == dataAccount[0].ToUpper() && p.Password == dataAccount[1]).Single();
            string res = UnitDatabase.TestAcount(DBase, dataAccount[3], ace, group, ConvertObject.ModeCode);
            if (res == "")
            {
                try
                {
